      ...
      <div className="w-full max-w-md">
        ...
        <form onSubmit={onSubmit} className="space-y-4 mb-6" noValidate>
          ...
          <div className="flex items-center">
            <input
              ...
              className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 dark:border-slate-600 dark:bg-slate-800 rounded disabled:opacity-50"
            />
            <label
              htmlFor="rememberMe"
        ...
        <div className="relative mb-6">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-2 bg-white dark:bg-slate-900 text-gray-500 dark:text-gray-400">
              {t("oauth.loginWithGoogle")?.includes("with") ? "Or" : "Or"}
            </span>
          </div>
        </div>
        ...
        <div className="space-y-2 mb-6">
          <button
            ...
            aria-label={t("oauth.loginWithGoogle")}
          >
            <span>🔵</span>
            {t("oauth.loginWithGoogle")}
          </button>
          <button
            onClick={() => onOAuthLogin("github")}
            disabled={loading}
            className="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors disabled:opacity-50 flex items-center justify-center gap-2 text-gray-700 dark:text-gray-300 font-medium"
            aria-label={t("oauth.loginWithGithub")}
          >
            <span>⚫</span>
            {t("oauth.loginWithGithub")}
          </button>
          <button
            onClick={() => onOAuthLogin("microsoft")}
            disabled={loading}
            className="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors disabled:opacity-50 flex items-center justify-center gap-2 text-gray-700 dark:text-gray-300 font-medium"
            aria-label={t("oauth.loginWithMicrosoft")}
          >
            <span>🪟</span>
            {t("oauth.loginWithMicrosoft")}
          </button>
        </div>
