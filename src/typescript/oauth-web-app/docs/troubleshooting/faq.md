...
Edit `.env.local` and set required variables (use `NEXT_PUBLIC_` prefix for public values):
...
The app will be available at `http://localhost:3000` with live reloading.

---

### Q: How do I add a new component?

**A**: Create a React component under `src/components/` and export it for use. Example structure:
...
Use `use client` in client components and import hooks or stores from `src/stores` or `src/lib`.
...
- **Hooks/Stores**: `useSomething.ts` / `store.ts` (e.g., `useAuth.ts`, `auth.store.ts`)
- **Utilities**: `camelCase` or descriptive filenames (e.g., `formatDate.ts`)
...
```typescript
// ✅ Correct: Absolute paths configured via tsconfig.json paths
...
Configure `tsconfig.json` for path aliases if desired.
...
**A**: Use async utilities from Jest and React Testing Library:
...
See [Testing Guide - Async](../guide/development/testing.md).
...
1. Create translation file under `public/locales/<lang>/common.json` (e.g., `public/locales/ar/common.json`).
2. Add translation keys and values to the JSON file.
3. Ensure the language is included in the language list used by the language switcher (Zustand store or config).
